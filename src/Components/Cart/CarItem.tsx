...
const CarItem = ({ item, productSelected, isCreated, setProduct }: any) => {
  ...
  const { updateProductOrder, flavorsList, setIsModalOpen } = useProductContext();
  ...
  return (
    <div key={item.id} onClick={() => setProduct(item)}>
      ...
      <div className={`fixed inset-0 z-[60] flex items-center justify-center ${openModal ? '' : 'hidden'}`}>
        <div
